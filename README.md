...
**`poetry new pythonversioncontrol`** — scaffold a new project (run once)
```bash
poetry new pythonversioncontrol
cd pythonversioncontrol
```
...
───────────                         ─────────────────────────
poetry new pythonversioncontrol                poetry install
  ↓                                   ↓
poetry add python-dotenv            import pythonversioncontrol
...
```python
# src/pythonversioncontrol/__init__.py
...
```bash
pip install sphinx
sphinx-apidoc -o docs/source src/pythonversioncontrol
...
To use `pythonversioncontrol` in a different project, install it there — it is not a live link. If the original package changes, you must reinstall to get the updates.

**Install from GitHub:**
```bash
pip install git+https://github.com/benjamincontrino/pythonversioncontrol.git

# Or with Poetry in the consuming project:
poetry add git+https://github.com/benjamincontrino/pythonversioncontrol.git
```

**Install from a local path:**
```bash
pip install -e /path/to/pythonversioncontrol    # -e = editable mode (live link, dev only)
poetry add --editable /path/to/pythonversioncontrol
```
...
pip install --upgrade git+https://github.com/benjamincontrino/pythonversioncontrol.git
# or with Poetry:
poetry update pythonversioncontrol
...
```bash
# 1. Clone the repo
git clone https://github.com/benjamincontrino/pythonversioncontrol.git
cd pythonversioncontrol
...
# 4. Open in VS Code
code pythonversioncontrol.code-workspace
...
```bash
# After editing src/pythonversioncontrol/ files:

# Run tests
poetry run pytest

# Build a distributable package (like devtools::build())
poetry build        # creates dist/pythonversioncontrol-0.1.0.tar.gz
...
├── pythonversioncontrol.code-workspace  ← VS Code settings (like .Rproj)
└── README.md
```
